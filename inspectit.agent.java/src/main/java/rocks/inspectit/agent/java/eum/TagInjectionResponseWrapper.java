...
/*
 * import javax.servlet.ServletOutputStream; import javax.servlet.http.HttpServletResponse; import
...
public class TagInjectionResponseWrapper implements IProxySubject {
	...
	private final W_HttpServletResponse wrappedResponse;
	private IRuntimeLinker linker;

	private final String tagToInject;
	...
	private boolean isNonHTMLContentTypeSet() {
		...
		if (contentMime != null) {
			...
			if (contentMime.startsWith("text/html") || contentMime.startsWith("application/xhtml+xml")) {
				return false;
			} else {
				return true;
			}
