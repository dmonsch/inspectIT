...
/**
 * A ServletOutputStream which injects the given tag on the fly into the head (or another
 * appropriate) section of the document. Automatically detects non-html and then falls back to just
 * piping the data through.
 *
 * @author Jonas Kunz
 */
...
public class TagInjectionOutputStream extends OutputStream implements IProxySubject {
	...
	private final W_ServletOutputStream originalStream;

	private final HTMLScriptInjector parser;
	...
	/**
	 * Sets the Character encoding used by the data. Only this way the stream is able to decode /
	 * encode binary data.
	 *
	 * @param charsetName
