...
public final class RuntimeLinker implements IRuntimeLinker {
	...
	/**
	 * Caches the generated Proxy classes 1st key (Class<?>) the class of the proxy subject (e.g.
	 * TagInjectionResponseWrapper) 2nd key (ClassLoader) the classloader which owns the proxy class
	 */
	private final ConcurrentHashMap<Class<?>, ConcurrentHashMap<ClassLoader, IProxyClassInfo>> linkedClassCache;
	...
	private <T> IProxyClassInfo getProxyClass(Class<T> subjectType, ClassLoader context, boolean create) {
		...
		if (classLoaderMap == null) {
			linkedClassCache.putIfAbsent(subjectType, new ConcurrentHashMap<ClassLoader, IProxyClassInfo>());
		...
		if (proxyClassInfo == UNLINKEABLE_MARKER) {
			return null;
		} else {
